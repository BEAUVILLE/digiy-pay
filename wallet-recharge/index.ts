...
serve(async (req) => {
  try {
    ...
    const { amount, method, module = "PAY_PRO", service = "RECHARGE", order_id = null, meta = {} } = body;
    ...
    const { data, error } = await sbAdmin.rpc("external_payment", {
      p_to_phone: auth.phone,
      p_amount: amt,
      p_method: method,
      p_module: module,
      p_service: service,
    ...
    if (error) return json(500, { ok: false, error: "wallet_recharge_failed", details: error.message });
